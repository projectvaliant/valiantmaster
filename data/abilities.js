...
let BattleAbilities = {
	...
	"comeback": {
		desc: "When this Pokemon has more than 2/5 its maximum HP and takes damage from an attack bringing it to 2/5 or less of its maximum HP, its Attack or Special Attack is raised by 1 stage based on which the opponent is weaker against. This effect applies after all hits from a multi-hit move; Sheer Force prevents it from activating if the move has a secondary effect.",
		shortDesc: "This Pokemon's Atk or SpeAtk is raised by 1 when it reaches 2/5 or less of its max HP based on weakness.",
		onAfterMoveSecondary(target, source, move) {
			if (!source || source === target || !target.hp || !move.totalDamage) return;
			if (target.hp <= target.maxhp / 2.5 && target.hp + move.totalDamage > target.maxhp / 2.5) {
				this.boost({spa: 1});
				this.boost({atk: 1});
			}
			},
	...
	"fear": {
		...
		onStart(pokemon) {
			...
			for (const target of pokemon.side.foe.active) {
				...
				if (!activated) {
					this.add('-ability', pokemon, 'Fear', 'boost');
					activated = true;
				}
	...
	"instinct": {
		desc: "When this Pokemon has more than 2/5 its maximum HP and takes damage from an attack bringing it to 2/5 or less of its maximum HP, its Attack and Speed is raised by 1 stage. This effect applies after all hits from a multi-hit move; Sheer Force prevents it from activating if the move has a secondary effect.",
		shortDesc: "This Pokemon's Atk and Spd is raised by 1 when it reaches 2/5 or less of its max HP.",
		onAfterMoveSecondary(target, source, move) {
			if (!source || source === target || !target.hp || !move.totalDamage) return;
			if (target.hp <= target.maxhp / 2.5 && target.hp + move.totalDamage > target.maxhp / 2.5) {
				this.boost({atk: 1});
	...
	"tipsy": {
		desc: "On switch-in, this Pokemon gains evasion, but lowers accuracy",
		shortDesc: "On switch-in, this Pokemon gains evasion, but lowers accuracy",
		onStart(pokemon) {
